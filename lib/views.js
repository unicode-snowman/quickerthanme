var render    = require('./templates')
  ...
  , RaceState = require('./websocket').RaceState
...
exports.rules = maybelogin(function(req, resp) {
  render('rules.html', {}, req, resp)
})

exports.about = maybelogin(function(req, resp) {
  render('about.html', {}, req, resp)
})
...
exports.active_races = maybelogin(function(req, resp) {
  var races = Race.objects.filter({'started_on__gt': 1, 'ended_on__lte': 1})
  races.order_by = ['-started_on']

  races.on('data', function(the_races) {
    render('active_races.html', {
      'active_races': the_races
    }, req, resp)
  }).on('error', function(err) {
    console.error("Active races failed: ", err, err.stack)
    render('active_races.html', {
      'active_races': []
    }, req, resp)
  })
      ...
      User.objects.filter({participated_in__pk:race.pk})(function(err, racers) {
        ...
        for(var i = 0; i < len; ++i)
          race_state.add_ghost(racers[i], function(err, data) {
            err && console.error('cant add ghost')
            error = err || error  
            ++idx
            if(idx === len) {
              ready(error)
...
exports.race_create = login(function(req, resp) {
  ...
  if(req.method === 'POST') {
    ...
    if(req.body.type == 'multiplayer') {
      ...
      else if(parseInt(req.body.min_players, 10) < 2 || parseInt(req.body.min_players, 10) > 4) {
