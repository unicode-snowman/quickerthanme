...
RaceState.prototype.on_disconnect = function(user_state) {
  var self = this
  var place = self.finished.map(function(state, idx) { return user_state.pk === state.user.pk ? idx : null}).filter(function(x) { return x !== null })[0]
    ...
    , function(is_okay) {
      if(is_okay) {
        ...
        if(user_state.solution === self.questions.length) {
          ...
          user_state.socket.emit('correct',  self.questions[user_state.question].get_user_data(), user_state.question, self.questions.length)
          self.users.forEach(function(ustate) {
